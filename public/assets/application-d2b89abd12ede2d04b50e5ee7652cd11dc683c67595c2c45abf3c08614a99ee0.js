...
$(document).ready(function(){
	...
	$('.create-statement').click(function(){
		...
		.done(function(response){
			...
			section_container.find('[data-statement-type-id=' + statement_type_id +']').siblings('.sortable').append('<div class="statement-container" id="' + response.id + '" data-statement-id="' + response.id + '"><div class="statement-text">' + new_statement_content + '</div><div class="edit-statement-form-container"><textarea rows="4" cols="80" class="statement-text-editor"></textarea><div><button class="btn btn-sm btn-default save-statement">Save Changes</button> or <button class="btn btn-link btn-sm cancel-statement-update">Cancel</button></div></div></div>');
			section_container.find('.add-statement').show();
			var statement_container = section_container.find('#' + response.id);
			
			statement_container.on('mouseenter', showStatementControls);
			statement_container.on('mouseleave', hideStatementControls);
