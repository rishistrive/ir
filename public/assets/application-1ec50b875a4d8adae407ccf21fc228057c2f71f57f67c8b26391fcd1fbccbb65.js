...
$(document).ready(function(){
	...
	// TODO: make this code reuseable - it will be needed in the report view too
	...
	function deleteStatementClickHandler(that){
		...
		.done(function(){
			...
			if ( statement_container.siblings('.statement-type-label').data('statement-type-id') === 4 ) {
				if (statement_container.siblings('.statement-container').length === 0) {
					var section_container = get_section_container(that);
					get_section_container(that).find('.deficient').prop('checked', false);
				};
			};
			statement_container.remove();
	...
	$('.add-statement').click(function(){
		...
		.done(function(response){
			...
			if (response.length > 0) {
				/*
				$.each(response,function(index, statement){
					statement_library_container.append('<div class="row"><div class="col-sm-3"><span class="statement-type" data-statement-type-id="'+ statement.statement_type_id +'"></span>'+ statement.keyword +'</div><div class="col-sm-9"><span class="statement-content">'+ statement.content +'</span></div></div>');
				});
				*/
				statement_library_container.children('.row').click(function(){
					section_container.find('.new-statement-text-editor').val($(this).find('.statement-content').html());
					section_container.find('.statement-type-selector').val($(this).find('.statement-type').attr('data-statement-type-id'));
					section_container.find('.create-statement').focus();
				});
			} else {
				statement_library_container.append('<li>You have no statements in the library for this section.</li>');
			}
		})
		.fail(function(){
	...
	$('.library-filter').keyup(function(){
		...
		.done(function(response){
			...
			if (response.length > 0) {
				$.each(response,function(index, statement){
					statement_library_container.append('<hr><div class="row library-statement-selection-container" data-statement-type-id="' + statement.statement_type_id + '"><div class="col-sm-12"><div class="keyword-display"><span class="prompt">Keyword(s): </span>'+ statement.keyword +'<button class="btn btn-sm btn-primary pull-right">Select statement</button></div><span class="statement-content">'+ statement.content +'</span></div></div>');
				});
				statement_library_container.children('.row').click(function(){
					section_container.find('.new-statement-text-editor').val($(this).find('.statement-content').html());
					// TODO: evaluate the potential need to classify library statements by their statement type - I'm skeptical that we need to do this (as of 1/27/16)
					//section_container.find('.statement-type-selector').val($(this).data('statement-type-id'));
					section_container.find('.create-statement').focus();
				});
			} else {
	...
	$('.create-statement').click(function(){
		...
		.done(function(response){
			...
			section_container.find('[data-statement-type-id=' + statement_type_id +']').siblings('.sortable').append('<div class="statement-container" id="' + response.id + '" data-statement-id="' + response.id + '"><div class="row statement-display"><div class="col-sm-10 statement-text">' + new_statement_content + '</div><div class="col-sm-2 text-right"><a class="edit-statement" role="button" title="Edit statement">Edit</a><a class="delete-statement" role="button" title="Remove statement"><span class="glyphicon glyphicon-trash"></span></a></div></div><div class="edit-statement-form-container row"><div class="col-sm-12"><div class="row"><div class="col-sm-12"><div class="form-group"><textarea rows="10" cols="80" class="form-control statement-text-editor"></textarea></div></div></div><div class="row"><div class="col-sm-12"><div class="form-group"><button class="btn btn-sm btn-primary save-statement">Save</button><span class="between-button-text">or</span><button class="btn btn-link btn-sm cancel-statement-update">Cancel</button></div></div></div></div></div></div>');
			section_container.find('.add-statement').show();
			...
			if (statement_type_id == 4) {
				section_container.find('.deficient').prop('checked', true);
			}
			$('html, body').animate({scrollTop: section_container.offset().top},0);
	...
	$('.create-inspector-statement').click(function(){
		...
		.done(function(response){
			...
			section_container.find('.add-inspector-statement-container').before('<div id="inspectorStatement' + response.id + '" class="inspector-statement-container" data-id="' + response.id +'"><div class="row inspector-statement-display"><div class="col-sm-8 inspector-statement-content">' + response.content + '</div><div class="col-sm-2 keyword-content">' + response.keyword + '</div><div class="col-sm-2 text-right"><a class="edit-inspector-statement" role="button" title="Edit statement">Edit</a><a class="delete-inspector-statement" role="button" title="Delete statement"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></div></div><div class="edit-inspector-statement-form-container row"><div class="col-sm-12"><div class="row"><div class="col-sm-12"><div class="form-group"><label for="inspector-statement-text-editor">Statement content</label><textarea rows="10" cols="80" class="inspector-statement-text-editor form-control"></textarea></div></div></div><div class="row"><div class="col-sm-12"><div class="form-group"><label for="keyword-text-editor">Keyword(s)</label><input class="keyword-text-editor form-control" type="text"></div></div></div><div class="row"><div class="col-sm-12"><div class="form-group"><button class="btn btn-sm btn-primary save-inspector-statement">Save</button><span class="between-button-text">or</span><button class="btn btn-link btn-sm cancel-inspector-statement-update">Cancel</button></div></div></div></div>');
	...
	function toggleInspected(that){
		...
		.done(function(){
			// commented b/c I'm not sure we will always want to do this
			/*if (inspected) {
	...
	function toggleNotInspected(that){
		...
		$.ajax({url: '/sections/' + section_container.data('section-id'),
			...
			data: {not_inspected: notInspected}})
			       //inspected: inspected}})
		.done(function(){
			section_container.find('.inspected').prop('checked');
			display_message('Section updated', 'success', 1500);
		})
		.fail(function(){
			display_message('Section update failed', 'danger', 3500);
	...
	$('.edit-inspection-date').click(function(){
		var clientNameContainer = get_inspection_date_container(this);
		clientNameContainer.find('.display-inspection-date-container').hide();
		clientNameContainer.find('.inspection-date-update-form').show().find('.inspection-date').focus();
	});

	$('.cancel-inspection-date-update').click(function(){
		var clientNameContainer = get_inspection_date_container(this);
		clientNameContainer.find('.inspection-date-update-form').hide();
		clientNameContainer.find('.display-inspection-date-container').show();
	});

	$('.save-inspection-date-update').click(function(){
		var clientNameContainer = get_inspection_date_container(this);
		var reportContainer = get_report_container(this);
		var inspectionDate= clientNameContainer.find('.inspection-date').val();
		$.ajax({url: '/reports/' + reportContainer.data('id'),
			method: 'patch',
			data: {inspection_datetime: inspectionDate}})
		.done(function(response){
			clientNameContainer.find('.inspection-date-display').html(response.inspection_datetime);
			clientNameContainer.find('.inspection-date-update-form').hide();
			clientNameContainer.find('.display-inspection-date-container').show();
